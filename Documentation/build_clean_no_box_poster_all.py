...
def build_clean_no_box_all():
    print("=== BUILDING CLEAN NO-BOX POSTER FOR BOTH PDF & PPTX ===")
    ...
    # -------------------------------------------------------------------------
    # PART 1: GENERATE NO-BOX PDF (960 x 540 pt)
    # -------------------------------------------------------------------------
    POSTER_WIDTH = 960
    POSTER_HEIGHT = 540
    ...
    # Helper function for clean NO-BOX sections
    ...
    doc.build(story)
    print(f"[SUCCESS] Clean No-Box PDF Generated at: {target_pdf_final}")

    shutil.copyfile(target_pdf_final, target_pdf_main)
    shutil.copyfile(target_pdf_final, submission_pdf)

    # -------------------------------------------------------------------------
    # PART 2: GENERATE NO-BOX PPTX (KEFFI_POSTER.pptx)
    # -------------------------------------------------------------------------
    prs = Presentation()
    ...
    # Helper function for clean NO-BOX PPT sections
    def add_no_box_ppt_section(slide, heading_text, lines, left_in, top_in, width_in, height_in):
    ...
    # System Architecture Section (NO BOX SHAPE, just text title + image)
    ...
    print(f"[SUCCESS] Clean No-Box PPTX Generated at: {target_pptx_main}")

if __name__ == "__main__":
    build_clean_no_box_all()

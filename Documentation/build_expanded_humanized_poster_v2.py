...
def build_expanded_humanized_poster():
    print("=== BUILDING EXPANDED HUMANIZED POSTER (11 HIGHLIGHTS, 8 STEPS, 4 DATASETS, 2 I/O EXAMPLES) ===")
    ...
    # -------------------------------------------------------------------------
    # PART 1: A3 LANDSCAPE PDF (1190.55 x 841.89 pt) WITH ALL EXPANSIONS
    # -------------------------------------------------------------------------
    ...
    def make_times_section(heading_text, content_elements, width_pt):
        ...
        sec_story.append(HRFlowable(width="100%", thickness=1.5, color=DIVIDER_COLOR, spaceBefore=2, spaceAfter=5))
    ...
    c1_2 = [
        ...
        Paragraph("• <b>HIPAA-Compliant Privacy & Data Anonymization (NEW):</b> Enforces AES-256 end-to-end data encryption and strict patient ID pseudonymization to guarantee absolute confidentiality.", style_bullet),
        Paragraph("• <b>Interactive Real-Time Grounding Exercises (NEW):</b> Delivers embedded interactive breathwork, 5-4-3-2-1 sensory grounding, and muscle relaxation widgets directly inside the chat UI.", style_bullet),
    ]
    sec_highlights = make_times_section("2. HIGHLIGHTS OF THE PROJECT", c1_2, 375)

    col1_sections = [sec_abstract, Spacer(1, 5), sec_highlights]

    # COLUMN 2 (CENTER: 390pt) - DEEPLY EXPANDED METHODOLOGY, 8 STEPS, & 4 DATASETS!
    ...
    # 8 ALGORITHM STEPS (2 NEW STEPS ADDED!)
    c2_2 = [
        ...
        Paragraph("<b>Step 7: Explainable AI Attribution Computation (NEW):</b> Calculate SHAP and LIME token attribution heatmaps to provide transparent rationale for psychiatrists.", style_bullet),
        Paragraph("<b>Step 8: Automated Patient Re-Engagement (NEW):</b> Dispatch gentle automated push notifications when a patient misses consecutive daily check-ins.", style_bullet),
    ]
    sec_procedure = make_times_section("4. STEP BY STEP PROCEDURE OR ALGORITHM", c2_2, 390)

    # 4 DATASETS (2 NEW DATASETS ADDED!)
    c2_3 = [
        ...
        Paragraph("• <b>EmpatheticDialogues Corpus (NEW):</b> 25,000 emotionally grounded conversational interactions used to fine-tune natural, empathetic Rogerian validation responses.", style_bullet),
        Paragraph("• <b>Behavioral Health Q&A Corpus (NEW):</b> Expert clinical psychology Q&A datasets utilized to calibrate Tier-2 neurobiological psychoeducation responses.", style_bullet),
    ]
    sec_dataset = make_times_section("5. DATASET USED IF ANY", c2_3, 390)

    col2_sections = [sec_methodology, Spacer(1, 5), sec_procedure, Spacer(1, 5), sec_dataset]
    ...
    c3_1 = [
        ...
        Paragraph("<b>Scenario 2: Acute Crisis & Suicide Risk Triage (NEW Point!)</b><br/>• <b>User Input:</b> <i>\"I can't take this emotional pain anymore, I feel like ending everything tonight.\"</i><br/>• <b>Immediate Safety Response:</b> Displays 24/7 national helpline contacts (988), triggers automated n8n emergency webhook alerts to designated supervisor desks, and initiates supportive audio voice readout.", style_bullet),
    ]
    ...
    col3_sections = [sec_io, Spacer(1, 4), sec_conclusion, Spacer(1, 4), sec_system_img]
    ...
    doc.build(story)
    print(f"[SUCCESS] Expanded Humanized PDF Generated at: {target_pdf_final}")

    shutil.copyfile(target_pdf_final, target_pdf_main)
    shutil.copyfile(target_pdf_final, submission_pdf)

    # -------------------------------------------------------------------------
    # PART 2: A3 PPTX (KEFFI_POSTER.pptx) WITH ALL EXPANSIONS
    # -------------------------------------------------------------------------
    prs = Presentation()
    ...
    def add_spacious_ppt_section(slide, heading_text, lines, left_in, top_in, width_in, height_in):
        ...
        for line in lines:
            ...
            pl.line_spacing = 1.18
            pl.space_after = Pt(3)
            pl.alignment = PP_ALIGN.LEFT

    # Column 1 (Left) - ABSTRACT & 11 HIGHLIGHTS
    ...
    add_spacious_ppt_section(
        ...
        [
            ...
            "• HIPAA-Compliant Privacy & Data Anonymization (NEW): Enforces AES-256 end-to-end encryption and patient ID pseudonymization.",
            "• Interactive Real-Time Grounding Exercises (NEW): Embedded interactive breathwork and sensory grounding widgets inside the chat UI."
        ],
        0.4, 4.6, 5.0, 6.7
    )

    # Column 2 (Center) - METHODOLOGY, 8 STEPS, & 4 DATASETS
    add_spacious_ppt_section(
        slide,
        "3. METHODOLOGY",
    ...
    add_spacious_ppt_section(
        ...
        [
            ...
            "Step 7: Explainable AI Attribution Computation (NEW): Calculate SHAP and LIME token attribution heatmaps for psychiatrist auditing.",
            "Step 8: Automated Patient Re-Engagement (NEW): Dispatch gentle automated check-in notifications when patients miss daily updates."
        ],
        5.7, 4.6, 5.2, 4.1
    )
    ...
    add_spacious_ppt_section(
        ...
        [
            ...
            "• EmpatheticDialogues Corpus (NEW): 25,000 emotionally grounded conversational interactions fine-tuning empathetic validation.",
            "• Behavioral Health Q&A Corpus (NEW): Expert clinical psychology Q&A datasets calibrating Tier-2 neurobiological psychoeducation."
        ],
        5.7, 8.8, 5.2, 2.5
    )

    # Column 3 (Right) - 2 INPUT/OUTPUT EXAMPLES, CONCLUSION, & SYSTEM INTERFACE
    add_spacious_ppt_section(
        slide,
        "6. INPUT AND OUTPUT",
        [
            ...
            "Scenario 2: Acute Crisis & Suicide Risk Triage (NEW)",
    ...
    print(f"[SUCCESS] Expanded Humanized PPTX Generated at: {target_pptx_main}")

if __name__ == "__main__":
    build_expanded_humanized_poster()

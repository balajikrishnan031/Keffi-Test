...
def build_spacious_easy_read_poster():
    print("=== BUILDING SPACIOUS EASY-TO-READ A3 POSTER (GENEROUS LINE GAPS & WORD-BY-WORD CLARITY) ===")
    ...
    # -------------------------------------------------------------------------
    # PART 1: SPACIOUS A3 LANDSCAPE PDF (1190.55 x 841.89 pt)
    # -------------------------------------------------------------------------
    ...
    # Generous line leading: 17.5pt for 10.5pt text (leaves ~7pt gap between lines!)
    ...
    # Generous bullet leading: 16.5pt for 10.0pt text
    ...
    # Helper function for spacious Times New Roman sections
    ...
    c1_2 = [
        Paragraph("• <b>BERT 96-Emotion Classifier:</b> Accurately identifies 96 distinct emotional states from complex user text with 94.2% top-3 accuracy.", style_bullet),
        Paragraph("• <b>3-Tier Clinical Care Framework:</b> Combines empathetic validation, biological explanations (amygdala and cortisol surges), and CBT grounding exercises.", style_bullet),
        Paragraph("• <b>Hands-Free Real-Time Voice AI:</b> Provides real-time speech conversation for users in acute panic who cannot type on keyboards.", style_bullet),
        Paragraph("• <b>High-Concurrency WAL Database:</b> Uses a Write-Ahead Logging database engine to prevent crashes during peak concurrent usage.", style_bullet),
        Paragraph("• <b>Explainable AI (SHAP & LIME):</b> Generates visual feature attribution maps that allow attending clinicians to audit automated decisions.", style_bullet),
        Paragraph("• <b>Admin Clinical Dashboard:</b> Enables psychiatrists to monitor patient progress, view inactivity alerts, and inspect full chat history.", style_bullet),
    ]
    ...
    doc.build(story)
    print(f"[SUCCESS] Spacious Easy-to-Read A3 PDF Generated at: {target_pdf_final}")

    shutil.copyfile(target_pdf_final, target_pdf_main)
    shutil.copyfile(target_pdf_final, submission_pdf)

    # -------------------------------------------------------------------------
    # PART 2: SPACIOUS A3 TIMES NEW ROMAN PPTX (KEFFI_POSTER.pptx)
    # -------------------------------------------------------------------------
    prs = Presentation()
    ...
    # Helper function for spacious Times New Roman PPT sections
    def add_spacious_ppt_section(slide, heading_text, lines, left_in, top_in, width_in, height_in):
        ...
        for line in lines:
            ...
            pl.space_after = Pt(6)  # Generous line spacing in PowerPoint text frames
    ...
    add_spacious_ppt_section(
        ...
        [
            "• BERT 96-Emotion Classifier: Accurately identifies 96 distinct emotional states from complex user text with 94.2% top-3 accuracy.",
            "• 3-Tier Clinical Care Framework: Combines empathetic validation, biological explanations (amygdala and cortisol surges), and CBT grounding exercises.",
            "• Hands-Free Real-Time Voice AI: Provides real-time speech conversation for users in acute panic who cannot type on keyboards.",
            "• High-Concurrency WAL Database: Uses a Write-Ahead Logging database engine to prevent crashes during peak concurrent usage.",
            "• Explainable AI (SHAP & LIME): Generates visual feature attribution maps that allow attending clinicians to audit automated decisions.",
            "• Admin Clinical Dashboard: Enables psychiatrists to monitor patient progress, view inactivity alerts, and inspect full chat history."
        ],
        0.4, 4.6, 5.0, 6.5
    )
    ...
    add_spacious_ppt_section(
        ...
        [
            ...
            "System Output:",
            "• Tier 1 Validation: 'I hear how much pressure you're under. It is completely valid to feel overwhelmed.'",
    ...
    print(f"[SUCCESS] Spacious Easy-to-Read A3 Times New Roman PPTX Generated at: {target_pptx_main}")

if __name__ == "__main__":
    build_spacious_easy_read_poster()

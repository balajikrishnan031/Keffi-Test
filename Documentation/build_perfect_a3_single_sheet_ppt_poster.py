...
def build_single_sheet_a3_poster():
    print("=== BUILDING PERFECT SINGLE-SHEET A3 PPT POSTER (NUMERICAL ORDER 1-7, NO BOXES) ===")
    ...
    # Standard A3 Landscape Page Dimensions: 1190.55 x 841.89 pt
    ...
    PRIMARY_GREEN = colors.HexColor('#0F3838')  # Deep forest green banner
    TEAL_HEADING = colors.HexColor('#0D5050')   # Heading color
    ...
    # 1. TOP HEADER BANNER (CORNER-TO-CORNER WITH BOTH LOGOS)
    header_html = """
    ...
    img_l = Image(img_logo_left, width=82, height=82) if os.path.exists(img_logo_left) else Paragraph("", style_body)
    img_r = Image(img_logo_right, width=82, height=82) if os.path.exists(img_logo_right) else Paragraph("", style_body)
    ...
    col1_topics = [sec_abstract, Spacer(1, 12), sec_highlights]
    ...
    sec_methodology = make_no_box_topic("3. METHODOLOGY", [c2_1], 390)
    ...
    c2_2 = [
        ...
        Paragraph("<b>Step 4: 3-Tier Response Synthesis:</b> Synthesize Rogerian validation, neurobiological explanations (cortisol/amygdala), and CBT grounding exercises.", style_bullet),
        ...
        Paragraph("<b>Step 6: Clinician Dashboard Sync:</b> Update Mental Health Quotient (MHQ) score telemetry and log complete transcripts in Admin Hub.", style_bullet),
    ]
    sec_procedure = make_no_box_topic("4. STEP BY STEP PROCEDURE OR ALGORITHM", c2_2, 390)
    ...
    sec_dataset = make_no_box_topic("5. DATASET USED IF ANY", c2_3, 390)

    col2_topics = [sec_methodology, Spacer(1, 10), sec_procedure, Spacer(1, 10), sec_dataset]
    ...
    sec_conclusion = make_no_box_topic("7. CONCLUSION", [c3_2], 370)

    # Embedding Live Landing Page UI & System Architecture Images
    img_ui_element = Image(img_hero, width=365, height=180) if os.path.exists(img_hero) else Paragraph("", style_body)
    img_arch_element = Image(img_arch, width=365, height=125) if os.path.exists(img_arch) else Paragraph("", style_body)
    
    sec_system_img = make_no_box_topic("LIVE PLATFORM INTERFACE & SYSTEM ARCHITECTURE", [img_ui_element, Spacer(1, 4), img_arch_element], 370)

    col3_topics = [sec_io, Spacer(1, 8), sec_conclusion, Spacer(1, 8), sec_system_img]

    # ASSEMBLE 3 COLUMNS SIDE BY SIDE ON A3 SHEET
    main_grid = Table([[col1_topics, col2_topics, col3_topics]], colWidths=[375, 395, 375])
    ...
    doc.build(story)
    print(f"[SUCCESS] Perfect Single-Sheet A3 PPT Poster Generated at: {target_pdf_final}")

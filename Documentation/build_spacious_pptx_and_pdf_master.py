...
def build_spacious_pptx_and_pdf():
    print("=== BUILDING PERFECT SPACIOUS PPTX & PDF POSTER (GENEROUS PPT LINE SPACING 1.35x & PDF LEADING 18pt) ===")
    ...
    # Generous 18.0pt leading for 10.0pt body text (~8pt gap between lines)
    ...
    # Generous 17.0pt leading for 9.8pt bullet points
    ...
    doc.build(story)
    print(f"[SUCCESS] Spacious A3 PDF Generated at: {target_pdf_final}")

    shutil.copyfile(target_pdf_final, target_pdf_main)
    shutil.copyfile(target_pdf_final, submission_pdf)

    # -------------------------------------------------------------------------
    # PART 2: PERFECT SPACIOUS A3 TIMES NEW ROMAN PPTX (KEFFI_POSTER.pptx)
    # -------------------------------------------------------------------------
    prs = Presentation()
    ...
    # Helper function for spacious Times New Roman PPT sections WITH 1.35x LINE SPACING & Pt(8) SPACE AFTER
    def add_spacious_ppt_section(slide, heading_text, lines, left_in, top_in, width_in, height_in):
        ...
        for line in lines:
            ...
            pl.line_spacing = 1.35   # Generous 1.35x line spacing inside PowerPoint!
            pl.space_after = Pt(8)    # Clean space after each paragraph/bullet!
    ...
    print(f"[SUCCESS] Spacious Master PPTX Generated at: {target_pptx_main}")

if __name__ == "__main__":
    build_spacious_pptx_and_pdf()

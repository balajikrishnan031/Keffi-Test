...
def build_white_theme_video():
    print("=== BUILDING 100% PURE WHITE THEME PROJECT JOURNEY DEMO VIDEO WITH SOFT HUMAN VOICEOVER ===")

    work_dir = r"e:\Keffi Ai"
    temp_dir = os.path.join(work_dir, "white_video_temp")
    ...
    # Generate SOFT HUMAN TONED VOICE OVER (Microsoft Zira tuned to 135 wpm rate, soft gentle tone)
    print("Generating Soft-Toned Human Conversational Voiceovers...")
    engine = pyttsx3.init()
    ...
    engine.setProperty('rate', 135) # Soft conversational pace
    ...
    for sc in scenes:
        ...
        print(f"Scene {sc['id']} Soft Voice Audio Generated: {duration:.2f}s (Frame Duration: {durations[-1]:.2f}s)")
    ...
    master_wav = os.path.join(temp_dir, "master_soft_narration.wav")
    ...
    print("Master Soft Narration Audio Concatenated Successfully!")

    # 100% PURE WHITE THEME FRAME RENDERING (1920x1080)
    print("Rendering 1080p 100% PURE WHITE THEME slide images...")
    ...
    for sc in scenes:
        ...
        # Subtle dark teal top rule line
        draw.line([(60, 130), (width-60, 130)], fill=(44, 85, 85), width=3)
        draw.line([(60, height-70), (width-60, height-70)], fill=(44, 85, 85), width=2)

        # Typography Fonts
        try:
            font_title = ImageFont.truetype("timesbd.ttf", 44)
            font_sub = ImageFont.truetype("timesi.ttf", 24)
            font_badge = ImageFont.truetype("timesbd.ttf", 22)
            font_text = ImageFont.truetype("times.ttf", 25)
            font_footer = ImageFont.truetype("times.ttf", 19)
        except:
        ...
        # Header Titles (Pure Black & Dark Teal Times New Roman)
        draw.text((60, 30), f"KEFFI AI — PROJECT JOURNEY DEMO | SCENE {sc['id']}/7", fill=(58, 112, 112), font=font_sub)
        draw.text((60, 65), sc['title'], fill=(0, 0, 0), font=font_title)

        # Footer Text
        draw.text((60, height-50), "TNSDC Naan Mudhalvan Niral Thiruvizha 3.0 | Project ID: NMNTSTD42260064", fill=(60, 60, 60), font=font_footer)
        draw.text((width-460, height-50), "Anna University Panruti | Team Hackers", fill=(44, 85, 85), font=font_footer)

        # Image/Diagram or Graphic Card Placement
        photo_path = sc['photo']
        if photo_path and os.path.exists(photo_path):
            try:
                photo_img = Image.open(photo_path).convert("RGB")
                photo_img = photo_img.resize((840, 610), Image.Resampling.LANCZOS)
                img.paste(photo_img, (60, 160))
                # Clean Dark Teal Photo Border
                draw.rectangle([(55, 155), (905, 775)], outline=(44, 85, 85), width=3)
            except Exception as e:
                print("Photo load error:", e)

            # Right Info Panel (Pure White with Subtle Border)
            draw.rectangle([(940, 160), (1860, 770)], fill=(250, 252, 250), outline=(143, 169, 137), width=2)
            draw.text((970, 190), sc['subtitle'], fill=(44, 85, 85), font=font_sub)

            # Badge Box
            draw.rectangle([(970, 245), (1830, 305)], fill=(44, 85, 85))
            draw.text((990, 260), sc['badge'], fill=(255, 255, 255), font=font_badge)
            ...
            y_off = 340
            for b in bullets:
                draw.text((970, y_off), b, fill=(0, 0, 0), font=font_text)
                y_off += 52

        else:
            # Full Widescreen White Card
            draw.rectangle([(60, 160), (1860, 770)], fill=(250, 252, 250), outline=(44, 85, 85), width=2)
            draw.text((100, 190), sc['subtitle'], fill=(44, 85, 85), font=font_title)

            # Badge Box
            draw.rectangle([(100, 270), (1200, 330)], fill=(44, 85, 85))
            draw.text((120, 285), sc['badge'], fill=(255, 255, 255), font=font_badge)
            ...
            y_off = 370
            for line in lines:
                draw.text((100, y_off), line, fill=(0, 0, 0), font=font_text)
                y_off += 60

        sc_png = os.path.join(temp_dir, f"white_scene_slide_{sc['id']}.png")
        img.save(sc_png)
        scene_images.append((sc_png, durations[sc['id']-1]))

    # FAST ENCODING VIA FFMPEG IN UNDER 10 SECONDS
    print("Muxing pure white scene slides and soft human narration into final 1080p MP4...")
    ...
    print(f"[SUCCESS] 2:30 Minute Pure White Theme Video with Soft Voice Over Rendered & Saved at:\n  1. {out_mp4_1}\n  2. {out_mp4_2}")

if __name__ == "__main__":
    build_white_theme_video()

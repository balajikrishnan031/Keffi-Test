...
def build_keffi_poster_with_logo():
    print("=== GENERATING OFFICIAL KEFFI AI PROJECT POSTER WITH LOGO ===")
    ...
    logo_path = r'e:\Keffi Ai\Documentation\extracted_report_images\page_1_img_1.png'
    ...
    BG_LIGHT = colors.HexColor('#F4F9F8')      # Light mint background
    ...
    # 1. HEADER BANNER TABLE WITH EMBEDDED LOGO
    header_text_html = """
    ...
    """
    p_header_text = Paragraph(header_text_html, style_title)

    if os.path.exists(logo_path):
        img_logo_left = Image(logo_path, width=1.5 * inch, height=1.5 * inch)
        img_logo_right = Image(logo_path, width=1.5 * inch, height=1.5 * inch)
        header_table = Table([[img_logo_left, p_header_text, img_logo_right]], colWidths=[1.8 * inch, 12.1 * inch, 1.8 * inch])
    else:
        header_table = Table([[p_header_text]], colWidths=[15.7 * inch])

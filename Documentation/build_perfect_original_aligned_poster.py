...
def build_perfect_original_aligned_poster():
    print("=== BUILDING PERFECT ORIGINAL ALIGNED POSTER (EXACT CONTAINERS, 1-7 ORDER, TIMES FONT, NEW IMAGE) ===")
    ...
    # -------------------------------------------------------------------------
    # PART 1: EXACT ORIGINAL ALIGNMENT PDF (960 x 540 pt)
    # -------------------------------------------------------------------------
    POSTER_WIDTH = 960
    POSTER_HEIGHT = 540
    ...
    DARK_GREEN = colors.HexColor('#0F3838')     # Original Header & Title Green
    ...
    # TOP HEADER BANNER WITH BOTH LOGOS
    header_html = """
    ...
    # USER'S NEW LANDING PAGE SCREENSHOT EMBEDDED
    sec_img_element = Image(img_user_landing, width=290, height=135) if os.path.exists(img_user_landing) else Paragraph("", style_body)
    card_system_img = make_card("DEVELOPED LIVE SYSTEM INTERFACE", [sec_img_element], 304)
    ...
    doc.build(story)
    print(f"[SUCCESS] Perfect Original Aligned PDF Generated at: {target_pdf_final}")

    shutil.copyfile(target_pdf_final, target_pdf_main)
    shutil.copyfile(target_pdf_final, submission_pdf)

    # -------------------------------------------------------------------------
    # PART 2: EXACT ORIGINAL ALIGNMENT PPTX (KEFFI_POSTER.pptx)
    # -------------------------------------------------------------------------
    prs = Presentation()
    prs.slide_width = Inches(13.33)  # 16:9 Aspect Ratio (960x540 equivalent)
    ...
    # Helper function for exact PPT card boxes
    ...
    # Developed Live System Interface Box Shape + Picture
    ...
    if os.path.exists(img_user_landing):
        slide.shapes.add_picture(img_user_landing, Inches(9.1), Inches(4.8), width=Inches(3.9), height=Inches(2.3))

    prs.save(target_pptx_main)
    shutil.copyfile(target_pptx_main, submission_pptx)
    print(f"[SUCCESS] Perfect Original Aligned PPTX Generated at: {target_pptx_main}")

if __name__ == "__main__":
    build_perfect_original_aligned_poster()

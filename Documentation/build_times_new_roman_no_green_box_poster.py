...
def build_times_roman_poster_all():
    print("=== GENERATING TIMES NEW ROMAN POSTER (NO TOP GREEN BOX, NO CARD BOXES) FOR PDF & PPTX ===")
    ...
    # -------------------------------------------------------------------------
    # PART 1: TIMES NEW ROMAN NO-BOX PDF (960 x 540 pt)
    # -------------------------------------------------------------------------
    POSTER_WIDTH = 960
    POSTER_HEIGHT = 540
    ...
    # TOP HEADER (NO GREEN BOX - CLEAN TEXT DIRECTLY ON WHITE BACKGROUND)
    header_html = """
    <font size="18" color="#0F3838"><b>KEFFI AI – A MENTAL HEALTH CHATBOT</b></font><br/>
    <font size="10" color="#0D5050"><b>UNIVERSITY COLLEGE OF ENGINEERING PANRUTI</b></font><br/>
    <font size="8" color="#334155"><i>(A Constituent College of Anna University, Chennai) • Department of Computer Science and Engineering</i></font><br/>
    <font size="7.5" color="#1E293B"><b>TEAM:</b> HACKERS TEAM &nbsp;|&nbsp; 
    ...
    def make_times_section(heading_text, content_elements, width_pt):
        ...
        sec_story.append(HRFlowable(width="100%", thickness=1.0, color=DIVIDER_COLOR, spaceBefore=1, spaceAfter=4))
    ...
    sec_system_img = make_times_section("DEVELOPED SYSTEM ARCHITECTURE", [c3_img_element], 304)
    ...
    doc.build(story)
    print(f"[SUCCESS] Times New Roman PDF Generated at: {target_pdf_final}")

    shutil.copyfile(target_pdf_final, target_pdf_main)
    shutil.copyfile(target_pdf_final, submission_pdf)

    # -------------------------------------------------------------------------
    # PART 2: TIMES NEW ROMAN NO-BOX PPTX (KEFFI_POSTER.pptx)
    # -------------------------------------------------------------------------
    prs = Presentation()
    ...
    # Header Textbox (NO TOP GREEN BOX - CLEAN TEXT ON WHITE)
    tx_head = slide.shapes.add_textbox(Inches(1.4), Inches(0.15), Inches(10.53), Inches(1.2))
    ...
    if os.path.exists(img_logo_left):
        slide.shapes.add_picture(img_logo_left, Inches(0.3), Inches(0.2), width=Inches(1.0), height=Inches(1.0))
    if os.path.exists(img_logo_right):
        slide.shapes.add_picture(img_logo_right, Inches(12.0), Inches(0.2), width=Inches(1.0), height=Inches(1.0))

    # Divider line under header
    line = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, Inches(0.3), Inches(1.35), Inches(12.73), Inches(0.02))
    line.fill.solid()
    ...
    tx_arch = slide.shapes.add_textbox(Inches(9.0), Inches(4.45), Inches(4.1), Inches(0.4))
    ...
    if os.path.exists(img_arch):
        slide.shapes.add_picture(img_arch, Inches(9.0), Inches(4.85), width=Inches(4.1), height=Inches(2.4))

    prs.save(target_pptx_main)
    shutil.copyfile(target_pptx_main, submission_pptx)
    print(f"[SUCCESS] Times New Roman PPTX Generated at: {target_pptx_main}")

if __name__ == "__main__":
    build_times_roman_poster_all()

...
def build_perfect_gap_spacious_poster():
    print("=== BUILDING PERFECT GENEROUS GAP SPACIOUS POSTER FOR PDF & PPTX ===")
    ...
    # -------------------------------------------------------------------------
    # PART 1: A3 LANDSCAPE PDF (1190.55 x 841.89 pt) WITH GENEROUS GAPS
    # -------------------------------------------------------------------------
    ...
    # Generous line leading: 15.5pt for 9.2pt text (leaves ~6.3pt gap between lines)
    ...
    # Generous bullet gaps: spaceAfter = 5.5pt
    ...
    # COLUMN 1 (LEFT: 375pt) - ABSTRACT & 11 HIGHLIGHTS
    ...
    col1_sections = [sec_abstract, Spacer(1, 8), sec_highlights]
    ...
    doc.build(story)
    print(f"[SUCCESS] Spacious Gap PDF Generated at: {target_pdf_final}")

    shutil.copyfile(target_pdf_final, target_pdf_main)
    shutil.copyfile(target_pdf_final, submission_pdf)

    # -------------------------------------------------------------------------
    # PART 2: A3 PPTX (KEFFI_POSTER.pptx) WITH GENEROUS GAPS (Pt(5) space_after & 1.22 line_spacing)
    # -------------------------------------------------------------------------
    prs = Presentation()
    ...
    def add_spacious_ppt_section(slide, heading_text, lines, left_in, top_in, width_in, height_in):
        ...
        for line in lines:
            ...
            pl.space_after = Pt(4)  # Generous paragraph spacing inside PPTX text frames
    ...
    print(f"[SUCCESS] Spacious Gap Master PPTX Generated at: {target_pptx_main}")

if __name__ == "__main__":
    build_perfect_gap_spacious_poster()

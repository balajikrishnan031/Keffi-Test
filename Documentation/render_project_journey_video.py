...
def build_journey_video():
    print("=== STARTING KEFFI AI 2:30 MINUTE PROJECT JOURNEY DEMO VIDEO RENDER ===")

    work_dir = r"e:\Keffi Ai"
    temp_dir = os.path.join(work_dir, "temp_video_build")
    ...
    # Script Scenes (7 Scenes totaling approx 150 seconds / 2:30 mins)
    scenes = [
        {
            "id": 1,
    ...
    # Generate TTS Voiceovers using pyttsx3
    ...
    engine.setProperty('rate', 150) # Moderate natural speech rate
    ...
    for sc in scenes:
        ...
        # Pad with 1 second silence at start and end
        audio_files.append(wav_path)
        durations.append(max(duration + 1.5, sc['target_duration']))
        print(f"Scene {sc['id']} Audio Generated: {duration:.2f}s (Allocated Frame Duration: {durations[-1]:.2f}s)")

    total_video_duration = sum(durations)
    print(f"Total Calculated Project Journey Video Duration: {total_video_duration:.2f} seconds ({total_video_duration/60:.2f} minutes)")
    ...
    print("Master Audio Narration Track Concatenated Successfully!")

    # Video Render Setup (1920x1080 @ 30 FPS)
    width, height = 1920, 1080
    fps = 30
    temp_avi = os.path.join(temp_dir, "video_frames_only.avi")

    fourcc = cv2.VideoWriter_fourcc(*'XVID')
    out_video = cv2.VideoWriter(temp_avi, fourcc, fps, (width, height))

    # Helper function to render a 1080p slide frame
    def render_scene_frame(scene, progress):
        # Base background: Deep Executive Teal Gradient (#1B3B3B to #2C5555)
        img = Image.new("RGB", (width, height), (27, 59, 59))
        draw = ImageDraw.Draw(img)

        # Draw decorative background panels
        draw.rectangle([(0, 0), (width, 140)], fill=(44, 85, 85)) # Header bar
        draw.rectangle([(0, height-80), (width, height)], fill=(20, 45, 45)) # Footer bar

        # Fonts (Fallback to default if specific font unavailable)
        try:
        ...
        # Header Text
        draw.text((60, 25), f"KEFFI AI — PROJECT JOURNEY DEMO | SCENE {scene['id']}/7", fill=(143, 169, 137), font=font_sub)
        draw.text((60, 65), scene['title'], fill=(255, 255, 255), font=font_title)
        ...
        # Main Body Layout (Left: Photo/Visual, Right: Narrative Info)
        photo_path = scene['photo']
        if photo_path and os.path.exists(photo_path):
            try:
                photo_img = Image.open(photo_path).convert("RGB")
                # Crop/resize to 800x600 card
            ...
            draw.text((970, 220), scene['subtitle'], fill=(212, 163, 115), font=font_sub)

            # Draw Badge
            draw.rectangle([(970, 280), (1830, 340)], fill=(58, 112, 112))
            draw.text((990, 295), scene['badge'], fill=(255, 255, 255), font=font_badge)
            ...
            # Full Widescreen Graphic Card
            draw.rectangle([(60, 180), (1860, 810)], fill=(38, 75, 75), outline=(143, 169, 137), width=3)
            draw.text((100, 220), scene['subtitle'], fill=(212, 163, 115), font=font_title)

            # Badge
            draw.rectangle([(100, 300), (1200, 365)], fill=(58, 112, 112))
            draw.text((120, 318), scene['badge'], fill=(255, 255, 255), font=font_badge)
        ...
        # Animated Progress Bar at bottom of screen
        prog_w = int(progress * (width - 120))
        draw.rectangle([(60, height-95), (60 + prog_w, height-88)], fill=(212, 163, 115))

        # Convert PIL to OpenCV BGR
        frame_bgr = cv2.cvtColor(np.array(img), cv2.COLOR_RGB2BGR)
        return frame_bgr

    print("Rendering 1080p video frames...")
    current_global_time = 0.0

    for sc_idx, sc in enumerate(scenes):
        sc_dur = durations[sc_idx]
        num_frames = int(sc_dur * fps)
        print(f"Rendering Scene {sc['id']}/7 ({num_frames} frames)...")

        for f_idx in range(num_frames):
            prog = (current_global_time + (f_idx / fps)) / total_video_duration
            frame = render_scene_frame(sc, prog)
            out_video.write(frame)

        current_global_time += sc_dur

    out_video.release()
    print("Video Frames Rendering Complete!")

    # Merge Video and Audio Tracks into Final MP4 using FFmpeg
    print("Muxing video frames and narration audio into final MP4...")
    cmd_mux1 = [
        ffmpeg_exe, "-y", "-i", temp_avi, "-i", master_wav,
        "-c:v", "libx264", "-pix_fmt", "yuv420p", "-c:a", "aac", "-b:a", "192k",
        "-shortest", out_mp4_1
    ]
    subprocess.run(cmd_mux1, check=True)
    ...
    subprocess.run(cmd_mux2, check=True)

    print(f"[SUCCESS] 2:30 Minute Project Journey Demo Video Rendered & Saved at:\n  1. {out_mp4_1}\n  2. {out_mp4_2}")
